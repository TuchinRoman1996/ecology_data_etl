...
def get_metadata(**kwargs):
    ...
    #     (request_id, status, load_date, table_name)
    #     VALUES({int(datetime.datetime.now().strftime("%Y%m%d%H%M%S"))},
    #             'LOAD',
...
def load_set_company_to_stg(**kwargs):
    ...
    cursor.execute(f"""
            INSERT INTO stg."DWH/DSO/2STGmetadata" (request_id, status, load_date, table_name)
            VALUES ({request_id}, 'LOAD', '{load_date}', '"DWH/AO/0companies"')
        """)

    conn.commit()

    with open('/mnt/c/Users/HW1/Desktop/Hadoop/companies_data (1).xml', 'rb') as xml_file:
...
def load_set_regions_to_stg(**kwargs):
    ...
    cursor.execute(f"""
        INSERT INTO stg."DWH/DSO/2STGmetadata" (request_id, status, load_date, table_name)
        VALUES ({request_id}, 'LOAD', '{load_date}', '"DWH/AO/0regions"')
    """)
    conn.commit()

    with open('/mnt/c/Users/HW1/Desktop/Hadoop/regions_data (1).xml', 'rb') as xml_file:
        parser = etree.iterparse(xml_file, events=('end',))
...
with DAG('load_set_from_xml_to_stg', default_args=default_args, schedule_interval='@once', catchup=False) as dag:
    ...
    get_metadata >> load_company_to_stg
    get_metadata >> load_regions_to_stg
