...
def load_set_metadata_to_stg(**kwargs):
    ...
    cursor.execute(f"""
        ...
        VALUES({requests_id}, 'LOAD', '{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}', '1STGemployees');
    """)
...
def load_employees_to_stg(**kwargs):
    ...
    with open('/mnt/c/Users/HW1/Desktop/Hadoop/employees_data (1).xml', 'rb') as xml_file:
...
with DAG('load_employees_from_xml_to_stg', default_args=default_args, schedule_interval='@once', catchup=False) as dag:
    ...
    load_metadata_to_stg >> load_employees_to_stg
